...
const generateFullSchemas = (eventType) => {
  ...
  const schemas = Events[eventClass].getSchemas().validation;
...
const generatePartialSchemas = (eventType) => {
  ...
  const schemas = Events.partials.auto[eventClass].getSchemas().validation;
  cleanAndWriteSchemas(schemas, eventName);
};
...
const cleanAndWriteSchemas = (schemas, eventName) => {
  ...
  if (eventName !== Events.Invalid.NAME) {
    if (_.has(schemas, 'properties.visit.properties.generated')) {
      delete schemas.properties.visit.properties.generated;
    }
  }

  // Remove these from the invalid event schemas
  delete schemas.properties.dateReceivedAsInvalid;

  // Do not require ID field on the event from the browser
  delete schemas.properties.id;
