...
const TrackerFactory = function(__schemas__, __sanitizeEvent__) {
  ...
  const Tracker = function (customerId, area, overridePixelUrl) {
    ...
    */
    self.__private.prepareAndSendEvent = (event, eventType) => {
      if (eventType === 'sessionChange') {
        // This event has been deprecated
        return;
      }

      event           = prepareEvent(event, eventType);
      const validated = self.__private.validateEvent(event, __schemas__[eventType]);
    ...
    */
    self.__private.validateEvent = (event, schemas) => {
      ...
      __sanitizeEvent__(sanitizedEvent, schemas);
    ...
    */
    self.__private.sendEvent = (event, sendSegment) => {
      ...
      const eventStringSegments = utils.chunkString(eventString, MAX_QUERY_STRING_LENGTH - SEGMENT_WRAPPER_OVERHEAD, LZString.compressToEncodedURIComponent);
