...
const Tracker = function(customerId, area, overridePixelUrl) {
  ...
  self.autoSetVisitor = (loginId) => {
    ...
    if (!visit.customerData.sessionId || visit.customerData.sessionId.length < 1) {
      visit.customerData.sessionId = cuid();
    }
    ...
    if (!visit.customerData.visitorId || visit.customerData.visitorId.length < 1) {
      visit.customerData.visitorId = cuid();
    }
   ...
   * @param event
   */
  self.sendMoreRefinementsEvent = (event) => self.__private.prepareAndSendEvent(event, 'autoMoreRefinements');
   ...
   * @param schema
   */
  self.__private.validateEvent = (originalEvent, schema) => {
    const event = utils.deepCopy(originalEvent);
    const result = inspector.validate(schema.validation, event);
    ...
    if (!event.visit.generated) {
      event.visit.generated = {};
    }

    event.visit.generated.uri = (typeof window !== 'undefined' && window.location) ? window.location.href : '';
    event.visit.generated.timezoneOffset = new Date().getTimezoneOffset();
    event.visit.generated.localTime = new Date().toISOString();
