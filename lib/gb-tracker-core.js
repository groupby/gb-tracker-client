...
let overriddenPixelUrl = null;
let overridenSearchUrl = null;
...
const Tracker = function (customerId, area, overridePixelUrl, overrideSearchUrl) {
  const self               = this;
  const customer           = {};
  const visit              = {customerData: {}};
  let invalidEventCallback = null;
  let strictMode           = false;
  overriddenPixelUrl       = overridePixelUrl || '';
  overriddenSearchUrl      = overrideSearchUrl || '';
  ...
  self.querySearchandiser = (query) => {
    const defaultSearchUrl = `${document.location.protocol }//${ customerId }.groupbycloud.com/search`;
    let searchUrl;

    if (overriddenSearchUrl && (typeof overriddenSearchUrl === 'string') && overriddenSearchUrl.length > 0) {
      searchUrl = overriddenSearchUrl;
    } else {
  ...
  self.__private.sendSegment = (segment) => {
    ...
    const im = new Image();
    if (overriddenPixelUrl && (typeof overriddenPixelUrl === 'string') && overriddenPixelUrl.length > 0) {
      im.src = overriddenPixelUrl + params;
    } else {
      im.src = host + path;
    }
  };
...
Tracker.__overridePixelPath = (path) => overriddenPixelUrl = path;
