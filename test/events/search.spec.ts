...
import { SearchEvent } from '../../src/models';
...
describe('search tests', () => {
  ...
  it('should accept valid search event and not lowercase navigations', (done) => {
    const expectedEvent = {
      search: {
        ...
        selectedNavigation: [
          {
            name: 'Refined 1', // not lowercase
            displayName: 'Refined 1', // not lowercase
            or: false,
            refinements: [
              {
                type: 'value',
                value: 'Something', // not lowercase
                count: 9823,
              },
            ],
          },
  ...
  it('should accept valid search event with zero-length search', (done) => {
    const expectedEvent = {
      search: {
        totalRecordCount: 10,
        pageInfo: {
  ...
  const lowercaseAllValues = (obj: any): any => {
    if (!_.isObject(obj)) {
      return;
    } else {
  ...
  it('should accept valid search event with records', (done) => {
    ...
    expectedEvent.search.records = expectedEvent.search.records.map((value: any) => {
