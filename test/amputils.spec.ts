...
function makeDocument(search: string): {
    location: {
        search: string;
    };
} {
...
describe('visitorIdFromAmpLocationSearch', () => {
    it('should return a visitor ID from an href "search" portion with a visitor ID encoded in an AMP Linker value, ignoring non-GroupBy AMP Linker params', () => {
        const document = makeDocument("?gbi=1*1u74ubq*gbivid*LU55Q2M3TkQyT2Y2VU5rMjlNekxpMXJjQ2g0MEYtNnRjbFhoaFhzLTQ0NDVYc0pXVWhiclFvNUNUbFNUMDFjSA..&_gl=1*18t1sx1*_ga*YW1wLXR0LXVWeHEzMjFzRExFcGVuYndNQWp5bDVUbm9ZSWo0eEZqR251TkFCazBMSE5oWjVHdVZpUm9qXzJvdXpaaDM.") as Document;

        const visitorId = visitorIdFromAmpLocationSearch(document, atob2);

        expect(() => visitorIdFromAmpLocationSearch(document, atob2)).to.not.throw;
